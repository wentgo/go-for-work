...
var accounts = []EmailAccount{
	{
		// emails from amazon.com
		Server:   os.Getenv("AUS-HOST"),
		Username: os.Getenv("AUS-USER"),
		Password: os.Getenv("AUS-PASS"),
	},
	{
		// emails from amazon.ca
		Server:   os.Getenv("ACA-HOST"),
		Username: os.Getenv("ACA-USER"),
		Password: os.Getenv("ACA-PASS"),
	},
	{
		// emails from bestbuy.ca
		Server:   os.Getenv("BB-HOST"),
		Username: os.Getenv("BB-USER"),
		Password: os.Getenv("BB-PASS"),
	},
	{
		// emails from all other marketplaces
		Server:   os.Getenv("OT-HOST"),
		Username: os.Getenv("OT-USER"),
		Password: os.Getenv("OT-PASS"),
	},
}
...
func main() {
	...
	// Setup Logger
	logfile := "c:/xampp/htdocs/btenew/app/logs/emails.log"
	...
	for _, account := range accounts {
		log.Printf("Connecting to %v as user %v.", account.Server, account.Username)
		conn, err := ConnectServer(account.Server, account.Username, account.Password)
		if err != nil {
			log.Printf("%s\n\n", err)
			continue
		}
		mails, err := FetchMails(conn, 100)
		if err == nil {
